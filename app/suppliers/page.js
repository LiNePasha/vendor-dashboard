                        ...
                        <div key={txn.id} className="text-xs">
                          <div className="flex justify-between items-start mb-1">
                            <div className="flex items-center gap-2 flex-1">
                              <span>{txn.type === 'purchase' ? '🛒' : '💵'}</span>
                              <span className="text-gray-700">{txn.description || (txn.type === 'purchase' ? 'شراء' : 'دفعة')}</span>
                            </div>
                            <span className={`font-bold ${txn.type === 'purchase' ? 'text-red-600' : 'text-green-600'}`}>
                              {txn.type === 'purchase' ? '-' : '+'}{parseFloat(txn.amount).toFixed(2)} ج.م
                            </span>
                          </div>
                          <div className="text-gray-400 text-[10px] mr-6">
                            ...
                            {' '}
                            🕐 {new Date(txn.date).toLocaleTimeString('ar-EG', { 
                              hour: '2-digit', 
                              minute: '2-digit',
                              hour12: true 
                            })}
